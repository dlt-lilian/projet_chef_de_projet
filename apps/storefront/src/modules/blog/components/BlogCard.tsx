...
export default function BlogCard({
  ...
  // Les champs sont maintenant plats (DB) — plus de `meta` imbriqué
  ...
  if (horizontal) {
    return (
      ...
      >
        <div className="relative w-24 h-16 flex-shrink-0 overflow-hidden bg-ui-bg-subtle">
          {cover && (
            <Image
              src={cover}
              alt={title}
        ...
        <div className="flex flex-col justify-between min-w-0">
          ...
          <p className="text-[11px] text-ui-fg-muted mt-1">
            {date} · {read_time}
          </p>
        </div>
      </Link>
  ...
  return (
    <Link href={`/blog/${slug}`} className="group flex flex-col">
      <div className="relative aspect-[16/10] overflow-hidden mb-4 bg-ui-bg-subtle">
        {cover && (
          <Image
            src={cover}
            alt={title}
